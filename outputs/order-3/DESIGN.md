...
One notes button opens a native modal dialog: a 420px drawer on desktop and an internally scrolling bottom sheet at 720px and below. The header and save/copy footer are flex siblings of the scroll region, so they remain visible without covering its last item. During a briefing, result or Help, the same trigger temporarily resides in that dialog's action area; otherwise it occupies its original topbar position. The native notes dialog is above both surfaces. This permits a note after victory or defeat without restarting. The closed board, ACTION ORDER, and hand retain their existing geometry, including the 155px hand at 700px and single-column hand at 320px. Card purpose metadata and combat rules are untouched.

`notes.js` owns an editor and an in-memory collection separate from `game`. It stops dialog click/keyboard propagation, uses native modality with explicit Tab boundaries, restores focus to the same notes control through a current-surface availability check, and never calls combat rendering, selection, mode, reset, queue, or execution functions. `capturePlaytestScene()` is the only integration point that reads combat context: it returns detached summaries of capture time, the single `GAME_VERSION = "ACT 16"` constant, turn, phase, selected card/mode/move actor, at most three orders, and the current/selection-before/event-after/final/resolving preview. No deck, complete units, forecast snapshots, movement evidence, cookies, query parameters, or other storage are collected. Opening an existing note preserves its attached scene until explicit refresh; attachment OFF stores `null`.
...
Validation: `node work/act15-test.js` compares every event's state, order, original outcomes/logs and final result with collection off/on and published commit 4b3c4123060b092f0259985c8d27055b59a742cb, also counting path/neighbor/adjacency/occupancy/random calls. `node work/act15-browser-test.js` checks T05, provenance navigation/focus, stale controls, selection, responsive layout, Help/notes, execution and touch emulation. Its `--help-regression` option invokes the maintained ACT14b suite. Historical ACT15 evidence remains under work/act15-implementation; later regressions use a separate ORDER3_ACT15_EVIDENCE directory. Full smoke retains ACT14a's death-boundary matrices with the current scene version expectation. These injected fixtures are implementation tests, not a new natural play observation. Physical browser zoom, actual assistive speech/touch hardware and human understanding remain unmeasured.

## Help within briefing and result screens (ACT 16)

The existing modal now has a stable action area containing the original primary control, a visible secondary “遊び方・全ての技” button and the one existing notes trigger. The secondary uses the same Help body and catalogue; it is hidden during Help. Explicit screen context, rather than title comparisons, determines “開始画面へ戻る”, “結果へ戻る” or “戦場へ戻る”. The existing presentation-only memento also preserves an unset initial action. Help returns to the original primary control without starting or restarting; a fresh explicit restart still calls the existing resetGame. No game or random state is saved/restored for Help.

Showing the modal moves the notes control before setting main inert. Closing it clears inert before reinserting that exact control at its original sibling position. The action area is not regenerated between views. Notes retains its listeners, count, draft, scene and storage/share procedures. Only its return-focus selection delegates to current-surface validation. All modal views circulate Tab/Shift+Tab through visible enabled controls, excluding closed-details descendants. Notes' native dialog takes priority: lower view changes do not move focus or handle its Escape/Tab. A new result supersedes an older Help return context while leaving an open editor or failed-save close choice untouched.

A display generation distinguishes held Enter/Space and pointer presses from fresh activation of the current primary/secondary control. A Help-closing event returns immediately; repeat keydowns and releases originating on an older view cannot activate the restored/new primary action. Fresh key or pointer activation remains available. Backdrop presses retain focus and do not dismiss initial/results. Ordinary focus restoration checks connection, visibility, disabled/inert and closed-details ancestors, with a current-surface fallback. These helpers operate only on presentation and input state.

At ordinary heights the existing body scroll remains, with a stable action area and wrapping 44px controls. At 500px height or less briefing/results can scroll the whole card. Help retains body scrolling and visible return/notes controls between 301px and 500px; below that it also permits whole-card scrolling. This keeps the text and actions reachable without shrinking text or covering the ending. New Help clears only its own disclosure keys and starts at its beginning; duplicate opens and notes retain the reading position. Result/briefing return preserves both body and card scroll positions. Ordinary battlefield/topbar geometry is preserved. ACT14a combat boundaries, ACT15 movement evidence and generation invalidation, all 24 effects and the complete Help body are unchanged.

`node work/act16-browser-test.js` covers visible pointer/tap/keyboard entry, contextual return, long-press/release boundaries, planning selections, seeded resolving with/without Help, notes failures/current results, and six standard/short viewports. `node work/act14b-fix-browser-test.js` retains its 13 behavioral groups, now using the in-modal visible entry, and compares combat/trace/Help contents against the ACT15 commit. ACT15 suites accept ORDER3_ACT15_EVIDENCE for separate regression evidence; notes-browser-test compares the full ACT15 product instead of removing controls from current HTML. ACT16 evidence is under work/act16-implementation. Injected terminal states are functional fixtures, distinct from the natural ACT16 observation. Actual zoom, screen-reader speech, hardware touch and human usability remain separate checks.

## Private feedback form (ACT 17)

The notes editor still uses the version 1 `order3.playtestNotes.v1` store, keeps old GitHub handoff fields untouched, and retains its Markdown/JSON copy and export paths. The visible send action now enters a confirmation view inside the existing notes dialog. It displays the exact plain-text response, including an optional detached scene and a fixed four-line record with kind, note ID, submission ID, and the current game version. Submission IDs are tied to the confirmed content and generated without combat randomness. A different body, kind, scene, or version gets a new ID. The old scene's capture version is never rewritten.

The Google Forms response URL and its observed `entry.1309058111` field are fixed in `notes-core.js`. An encoded URL of at most 1800 characters pre-fills the sole paragraph question. A longer response opens the clean responder URL and must be copied or exported in full, then pasted into that question. The original tab retains the selectable full text and draft. A new explicit click opens an `about:blank` tab, severs its opener, and navigates to the responder page. Only a returned popup handle counts as opened. The game never submits the form itself or claims success from opening, closing, or returning from the tab. The Google confirmation screen is the player's send receipt; the private Sheet intake is a separate production step.
...
## Pre-registration Interpose previews (ACT 22)

While a living Rook's unused Interpose is selected as a technique in planning with an open queue slot, each currently legal ally target receives a small, independent provisional forecast below the board. The forecast appends exactly one complete action to the current queue and uses `predictTimeline()` for the before and after snapshots. Its displayed route calls the existing `simPathToAdjacent()` on that event's before-state, applies the same two-step and rooted rules, and verifies the endpoint against the after-state. Cancelled outcomes show no successful route or guard. Candidate reading and focus change only short-lived display state; the selected path overlay never blocks board cells. A separate row button, or the existing legal board target, registers through `handleCellClick`. Row registration checks the current selected card instance, technique mode, turn/queue generation, target ID and location, and `validCells()` at activation. Old rows cannot register. Other cards, ALT movement, resolver, AI, draw and RNG are unchanged.
